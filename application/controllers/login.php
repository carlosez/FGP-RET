	...
	public function redireccion(){
        ...
        $jsondata['mensaje']    = "LOS CAMPOS DE USUARIO Y CONTRASEÑA SON OBLIGATORIOS";
         ...
         else
          {
             ...
             if ($this->ret_model->comprobar_usuario($_REQUEST['nick'], $_REQUEST['pass']))
             {  
                    $data=$this->ret_model->comprobar_usuario($_REQUEST['nick'], $_REQUEST['pass']);
                    $this->load->library('session');
                    $this->session->set_userdata($data);
                    $jsondata['bandera']    = 1;
                    $jsondata['nivel']    = "index.php/".$data['ID_TIPO']."";
